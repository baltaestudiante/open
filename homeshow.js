...
// Helper para obtener datos globales
const getData = () => window.DATA || [];

// ---------- Tarjetas individuales ----------
function createStandardCard(ep) {
    const isInPlaylistItem = window.isInPlaylist ? window.isInPlaylist(ep.mediaUrl) : false;
    ...
    return `<div class="card-std group">
        <div class="relative w-full aspect-square rounded-xl overflow-hidden bg-zinc-800" onclick="window.goToDetail('${ep.detailUrl}')">
            <img src="${ep.cover}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" loading="lazy">
            <div class="overlay-full">
                <img src="${addIcon}" class="action-icon" onclick="window.handleAdd(event, ${ep.id}); return false;" data-episode-id="${ep.id}" data-added="${isInPlaylistItem}">
...
function createVideoExpand(ep) {
    ...
    return `<div class="card-video group">
        <img src="${ep.cover}" class="absolute inset-0 w-full h-full object-cover z-10 group-hover:opacity-0 transition-opacity duration-300" loading="lazy">
        ${hasCover2 ? `<img src="${ep.coverWide}" class="absolute inset-0 w-full h-full object-cover opacity-0 group-hover:opacity-100 transition-opacity duration-300" loading="lazy">` : ''}
...
function createListItem(ep, idx) {
    ...
    return `<div class="list-item group">
        ...
        <div class="relative w-12 h-12 sm:w-14 sm:h-14 flex-shrink-0 rounded overflow-hidden" onclick="window.goToDetail('${ep.detailUrl}')">
            <img src="${ep.cover}" class="w-full h-full object-cover" loading="lazy">
...
function createGridCard(item) {
    const isInPlaylistItem = window.isInPlaylist ? window.isInPlaylist(item.mediaUrl) : false;
    const addIcon = isInPlaylistItem ? ICONS.added : ICONS.add;
    ...
    return `
        <div class="grid-card group">
            <div class="aspect-square bg-zinc-800 relative" onclick="window.goToDetail('${item.detailUrl}')">
                <img src="${item.cover}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" loading="lazy">
                <div class="overlay-full">
                    <img src="${addIcon}" class="action-icon" onclick="window.handleAdd(event, ${item.id}); return false;" 
                    ...
                    <img src="${dlIcon}" class="action-icon" onclick="window.handleDl(event, ${item.id}); return false;" 
...
export function renderFeed(container) {
    ...
    if (!DATA || DATA.length === 0) {
        container.innerHTML = '<div class="text-center py-20"><p class="text-gray-400 text-lg">No hay contenido disponible.</p></div>';
        return;
    }
    ...
    let html = '';
    html += createCarousel("Nuevos Lanzamientos", "standard", getRandomSafe(15, ep => new Date(ep.date) > new Date(Date.now() - 30*24*60*60*1000)), "Todos");
    html += createCarousel("Series de Video", "expand", getRandomSafe(10, e => e.type === 'video'), "Cine y TV");
    html += createCarousel("Top Semanal", "list", getRandomSafe(16), "Todos");
    html += createCarousel("Para Estudiar Profundamente", "double", getRandomSafe(20, e => e.categories.includes("Matemáticas") || e.categories.includes("Física y Astronomía")), "Matemáticas");
    html += createCarousel("Matemáticas", "standard", getRandomSafe(15, e => e.categories.includes("Matemáticas")), "Matemáticas");
    html += createCarousel("Especiales en Video", "expand", getRandomSafe(10, e => e.type === 'video' && e.categories.includes("Documentales")), "Documentales");
    html += createCarousel("Física y Astronomía", "standard", getRandomSafe(15, e => e.categories.includes("Física y Astronomía")), "Física y Astronomía");
    html += createCarousel("Ciencias Naturales y Tecnología", "double", getRandomSafe(20, e => e.categories.some(c => ["Ciencias Naturales", "Tecnología e Informática"].includes(c))), "Otras Ciencias");
    html += createSeriesCarousel();
    html += createCarousel("Otras Ciencias y Disciplinas", "standard", getRandomSafe(15, e => e.categories.includes("Otras Ciencias") || e.categories.some(c => ["Ciencias Naturales", "Tecnología e Informática"].includes(c))), "Otras Ciencias");
    container.innerHTML = html;
}

export function renderGrid(container, items, title) {
    const gridHtml = items.map(item => createGridCard(item)).join('');
    const emptyHtml = `<div class="text-center py-10"><p class="text-gray-400">No hay elementos para mostrar.</p></div>`;
    container.innerHTML = `
        <div class="flex items-center justify-between mb-6 sm:mb-8 mt-4 sm:mt-6">
            <h2 class="text-xl sm:text-2xl font-bold">${title}</h2>
            <button onclick="window.Router?.navigate('/')" class="text-sm font-bold text-gray-400 hover:text-white flex items-center gap-1">
                <span class="text-xl">×</span> Cerrar
            </button>
        </div>
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 sm:gap-6">
            ${items.length ? gridHtml : emptyHtml}
        </div>
    `;
}

export function renderSeries(container, episodes) {
                ...
                <div class="podcast-episode-actions">
                    <div class="podcast-left-episode-actions">
                        <button class="podcast-action-btn podcast-add-btn" data-mediaurl="${ep.mediaUrl}" title="Añadir a mi lista">
                            <img src="${window.isInPlaylist && window.isInPlaylist(ep.mediaUrl) ? ICONS.added : ICONS.add}" alt="añadir">
                        </button>
    ...
    return `
        <div class="podcast-widget">
                ...
                <div class="podcast-control-bar">
                    <div class="podcast-left-actions">
                        ...
                        <button class="podcast-icon-btn" id="episodeBtnAdd" title="Añadir a lista">
                            <img src="${window.isInPlaylist && window.isInPlaylist(episode.mediaUrl) ? ICONS.added : ICONS.add}" alt="">
                        </button>
            ...
            <div class="part-of-program">
                ...
                <div class="program-card" onclick="window.Router?.navigate('${series?.url_serie}')">
                    <img src="${series?.portada_serie || episode.cover}" alt="${series?.titulo_serie}">
                    <div>
                        <h3>${series?.titulo_serie || 'Serie'}</h3>
...
function attachSeriesEvents(episodes) {
    ...
    document.getElementById('podcastLastEpisodePlayBtn')?.addEventListener('click', (e) => {
        e.preventDefault();
        window.safePlayEpisode?.(lastEpisode) || window.handlePlay?.(e, lastEpisode.id);
    });
    document.getElementById('podcastBtnAddMain')?.addEventListener('click', () => {
        window.addToUserPlaylist?.({
    ...
    document.getElementById('podcastBtnDownloadMain')?.addEventListener('click', () => {
        if (lastEpisode.allowDownload) window.open(lastEpisode.mediaUrl, '_blank');
        else alert('Descarga no disponible');
    });
    ...
    episodes.forEach((ep, index) => {
        ...
        card.querySelector('.podcast-play-specific')?.addEventListener('click', (e) => {
            e.stopPropagation();
            window.safePlayEpisode?.(ep) || window.handlePlay?.(e, ep.id);
        });
        card.addEventListener('click', () => {
            window.Router?.navigate(ep.detailUrl);
        });
    });
}
...
function attachEpisodeEvents(episode) {
    ...
    document.getElementById('episodePlayBtn')?.addEventListener('click', (e) => {
        window.safePlayEpisode?.(episode) || window.handlePlay?.(e, episode.id);
    });
}

// Exponer funciones globalmente para que index.html pueda llamarlas
window.homeshow = {
    renderFeed,
    renderGrid,
    renderSeries,
